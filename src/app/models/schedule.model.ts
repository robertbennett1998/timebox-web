...
export default class ScheduleModel {
  ...
  constructor(private _date : Date, private _timeslotLengthInMins : number = 15) {
    ...
    for (let i = 0; i < ((((24 - this._date.getHours()) * 60) - this._date.getMinutes()) / this._timeslotLengthInMins); i++)
      this.generatedTimeboxes.push(new TimeboxModel(new Date(this._date.getFullYear(), this._date.getMonth(), this._date.getDate(), this._date.getHours(), this._date.getMinutes() + (15 * i)), this._timeslotLengthInMins, Math.floor(Math.random() * 101) >= 50 ? new ScheduableBase(Guid.create().toString()) : null));
  }
