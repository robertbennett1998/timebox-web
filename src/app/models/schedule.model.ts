...
export default class ScheduleModel {
  ...
  constructor(private _date : Date, private _timeslotLengthInMins : number = 15) {
    ...
    this.generatedTimeboxes.push(new TimeboxModel(new Date(this._date.getFullYear(), this._date.getMonth(), this._date.getDate(), this._date.getHours(), this._date.getMinutes() + (15 * i)), this._timeslotLengthInMins));

  for (let timebox of this.generatedTimeboxes)
    if (Math.floor(Math.random() * 101) >= 50)
      timebox.allocatedScheduable = new ScheduableBase("guid")
